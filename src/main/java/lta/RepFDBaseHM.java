...
public class RepFDBaseHM {
    ...
    public RepFDBaseHM(String filename, String path, double th, int w1, int w2) {
        this.threshold = th;
        this.w1 = w1;
        this.w2 = w2;
    ...
    public double[] execute() throws IOException, CsvException {
        ...
        // Initialize Level 0
        SimpleCombinationHelperHM chLevel0 = new SimpleCombinationHelperHM();
        ...
        String outPath =
                System.getProperty("user.dir") + "/repFDHM_result/" + filename + "_" + this.w1 + "-" + this.w2 + "-" + this.threshold + ".csv";
        ...
        switch (this.similarityFunc) {
            ...
            case 3://hamming distance
                ...
                for (int i = this.result_l.get(level).size(); i-- > 0; ) {
                    ...
                    double sim = lhs.cardinality();
                    sim = (double) Math.round((this.numberAttributes - sim) / this.numberAttributes * 100000) / 100000;
                    sim = sim * this.w1;
                    if (a == rhs) {
                        sim += 1 * this.w2;
                    }
    ...
    private void initializeCplusForLevel() {
        for (BitSet X : level1.keySet()) {
            ...
            //initialize simiFDs
            Int2ObjectOpenHashMap<ObjectOpenHashSet<FDResult>> simiFDs = new Int2ObjectOpenHashMap<>();
    ...
    private StrippedPartition getPartition(BitSet key) {
        ...
        for (int i = 0, j = 1; i < subLhs.size() - 1; i++, j++) {
            resultPartition = multiply(resultPartition, subLhs.get(j));//new StrippedPartition as the product of the
            // two given StrippedPartitions.
        }
        mmCombinedPartitions.addPartition(k, resultPartition);
        return resultPartition;
    }
    ...
    private void generateNextLevel() {
        ...
        Object2ObjectOpenHashMap<BitSet, SimpleCombinationHelperHM> new_level = new Object2ObjectOpenHashMap<BitSet,
                SimpleCombinationHelperHM>();
        buildPrefixBlocks();
    ...
    public static void main(String[] args) throws CsvException, IOException {
        ...
        RepFDBaseHM repFD = new RepFDBaseHM(fileName, filePath, 0.8, 1, 1);
