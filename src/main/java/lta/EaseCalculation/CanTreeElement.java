...
public class CanTreeElement {
    protected CanTreeElement[] children;
    public boolean hasMore;
    protected BitSet rhsAttributes;
    protected boolean[] isfd;
    protected StrippedPartition partition = null;
    protected int maxAttributeNumber;

    public CanTreeElement(int maxAttributeNumber) {
        this.maxAttributeNumber = maxAttributeNumber;
        children = new CanTreeElement[maxAttributeNumber];
        hasMore = false;
     ...
     * @return
     */
    public CanTreeElement getChild(int i) {
        return children[i];
    }
    ...
    public void addPartition(StrippedPartition p) {
        this.partition = p;
    }
    ...
    public boolean containsLHS(BitSet lhs, int currentAttr) {
        ...
        if (this.children[nextSetAttr - 1] != null) {
            ...
            found = children[nextSetAttr - 1].containsLHS(lhs, nextSetAttr);

        }
        if (!found) {
            return this.containsLHS(lhs, nextSetAttr);
        }
    ...
    public boolean getGeneralizationAndDelete(BitSet lhs, int a, CanTree root, int currentAttr, BitSet specLhs) {
        boolean found = false;
        int nextSetAttr;
        if (this.isfd[a - 1]) {
            this.isfd[a - 1] = false;
        ...
        if (this.children[nextSetAttr - 1] != null) {
            if (this.children[nextSetAttr - 1].getRhsAttributes().get(a)) {
                ...
                if (found) {
                    // 子节点的rhs中都没有a，则删除当前节点中的a，表示该路径之后不存在以a为右手边的FD
                    if (this.isFinalNode(a)) {
                        this.rhsAttributes.clear(a);
                    }
    ...
    public void filterSpecializations(CanTree filteredTree, BitSet activePath) {
        int attr;
        for (attr = 1; attr <= maxAttributeNumber; attr++) {
            if (children[attr - 1] != null) {
                activePath.set(attr);
        ...
        for (attr = 1; attr <= maxAttributeNumber; attr++) {
            if (this.isfd[attr - 1]) {
                ...
                if (!filteredTree.getSpecialization(activePath, attr, 0, new BitSet())) {
                    ...
                    System.out.println("+ " + activePath + " \\️ " + attr);
                }
            }
        }
    }
    ...
    public void filterGeneralizations(CanTree filteredTree, BitSet activePath) {
        int attr;
        for (attr = 1; attr <= maxAttributeNumber; attr++) {
            if (isfd[attr - 1]) {
